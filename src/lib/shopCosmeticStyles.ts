...
export function resolveWallpaperStyles(
  ...
  // Multi-media packs (anime/cars): use selected variant or first
  ...
  if (variants.length > 0) {
    const sel =
      ...
      (typeof meta?.["selected_variant_id"] === "string"
        ? String(meta["selected_variant_id"])
        : null);
    const chosen = variants.find((v) => v.id === sel) || variants[0];
    if (chosen?.url) {
      return { kind: chosen.kind, url: chosen.url, css: null };
    }
  }
